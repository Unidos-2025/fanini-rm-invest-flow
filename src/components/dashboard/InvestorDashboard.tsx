  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="financial-card">
          <CardHeader className="pb-2">
            <CardDescription className="text-emerald-200">Lucro Total</CardDescription>
            <CardTitle className="text-2xl font-bold text-emerald-400">
        ...
        <Card className="financial-card">
          <CardHeader className="pb-2">
            <CardDescription className="text-emerald-200">Rendimento Diário</CardDescription>
            <CardTitle className="text-2xl font-bold text-emerald-400">
              {dailyYield}%
            </CardTitle>
          </CardHeader>
        ...
        <CardContent>
          <div className="space-y-4">
            {[
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-emerald-500/20">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    activity.type === 'profit' ? 'bg-emerald-500/20' :
                    activity.type === 'reinvest' ? 'bg-cyan-500/20' :
                    'bg-red-500/20'
                  }`}>
                    {activity.type === 'profit' ? <ArrowUp className="h-4 w-4 text-emerald-400" /> :
                     activity.type === 'reinvest' ? <ArrowUp className="h-4 w-4 text-cyan-400" /> :
                     <ArrowDown className="h-4 w-4 text-red-400" />}
                  </div>
                  <div>
                    <p className="text-white font-medium">{activity.description}</p>
                    <p className="text-emerald-200 text-sm">{activity.date}</p>
                  </div>
                </div>
                <p className={`font-bold ${
                  activity.type === 'withdraw' ? 'text-red-400' : 'text-emerald-400'
                }`}>
